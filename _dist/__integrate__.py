...
def choiseWeaponType(files, repo):
    ...
    for file in files:
        if not file.endswith('_tp.mtl') and not file.endswith('_d.mtl') and not re.search('sa[0-9]{0,2}.mtl', file):
            ...
            parent_gun = sg.popup_get_text('base for new one?', default_text=gui_item)
            #-----------------------
...
#------------------------------------------------------------------------------------------
# Game\Items\Weapons\smg51.xml    (Special sockets)
#------------------------------------------------------------------------------------------
        elif re.search('sa[0-9]{0,2}', weapon_object):
...
#------------------------------------------------------------------------------------------
#                           Items/Weapons/smg51_shop.xml  (Shop xml)
#------------------------------------------------------------------------------------------
...
def Ask_Path():
    ...
    layout = [
        ...
        [sg.InputText(
            default_text = r"d:/__some_get_come__/__test_Weapon/Game",
            key = "-SKINS-"),
            sg.FolderBrowse()],
