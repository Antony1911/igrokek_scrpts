...
def Default_attach(i, weapon, skin, def_att, railwayStation):
    ...
    try:
        for elem in shutil_root.findall("content"):
            shutil_root.remove(elem)
    except:
        pass
    ...
    try:
        for elem in root_acc.find('UI_stats').findall('param'):
            ...
            if elem.attrib['name'] == "icon":
                elem.attrib['value'] = f"{weapon}_{def_att}"
    except AttributeError:
        pass
...
def KnShop(full_name, material):
    ...
    <UI_stats>
        <param name="name" value="@{full_name}_skin_name"/>
...
def KN_exist_It(kn_Files, It_SkinPath, sa_list):
    ...
    for elem in root.find("UI_stats").findall('param'):
        if elem.attrib['name'] == 'name':
            elem.attrib['value'] = f"@{full_name}_name"
        if elem.attrib['name'] == 'description':
...
def WeShop(item, material, full_name, def_attachment, def_attachment01 ):
    ...
    if os.path.exists(donor_shop):
        ...
        <UI_stats>
            ...
            <param name="name" value="@{item}_name"/>
            <param name="description" value="@{item}_desc"/>
    ...
    if exists == 0:
        ...
        for elem in shop_root.find('UI_stats').findall("param"):
            if elem.attrib['name'] == "name":
                elem.attrib['value'] = f"@{item}_name"
            if elem.attrib['name'] == "description":
                elem.attrib['value'] = f"@{item}_desc"
            if elem.attrib['name'] == "icon":
    ...
    if material == "default":
        ...
        sClass = item.strip('0123456789')
        if sClass == 'ar' or sClass == 'mg':
            sClass = 'au'
        ...
        <UI_stats>
            <param name="category" value="{sClass}"/>
            <param name="name" value="@{full_name}_name"/>
            <param name="description" value="@{full_name}_desc"/>
            <param name="icon" value="{full_name}"/>
        ...
        <mmo_stats>
            <param name="item_category" value="{skin_type}Skin"/>
            <param name="shopcontent" value="1"/>
            <param name="classes" value="{spec_class[0]}"/>
        </mmo_stats>
        <UI_stats>
            <param name="name" value="@{full_name}_skin_name"/>
            <param name="description" value="@{full_name}_skin_desc"/>
            <param name="icon" value="{full_name}"/>
        </UI_stats> 
        <skin>
            <param name="thumbnail_icon" value="{full_name}_thumbnail"/>
...
def WE_exist_It(kn_Files, It_SkinPath, sa_list):
    ...
    if len(kn_Files) < 1:
        ...
        if os.path.exists(wfc_repo + f"\\Items\\Weapons\\{item}_gold01.xml"):
            shutil.copyfile(wfc_repo + f"\\Items\\Weapons\\{item}_gold01.xml", skin_repo + f"\\Items\\Weapons\\{item}_gold01.xml")
            temp = [It_SkinPath, skin_repo + f"\\Items\\Weapons\\{item}_gold01.xml"]
            
            for xmlItem in temp:

                root = ET.parse(xmlItem).getroot()
                root.attrib['net_policy'] = "weapon"
        ...
        try:
            ...
            for param in root.find('drop_params').findall('item'):
                param.attrib['name'] = f"{item}_{material}"
...
if __name__ == "__main__":
    ...
    # skin_repo = os.path.abspath("d:\\__some_get_come__\\__test_Weapon\\sr50\\Game")
    # # ==================TEST_PATH==================================
