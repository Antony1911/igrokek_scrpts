...
#------------------------------------------------------------------------------
#                           Game\Objects\Attachments + "bp07"fix
#------------------------------------------------------------------------------
def obj_att_func(client_att, rep_att, item, temp, mtl):

    xml = '\\' + f'{item}' + '\\' + f'{item}.xml'
    ...
    def _next_step(path, mtl_list, weapon_folder):
        for x in mtl_list:

            weapon_skin_name = x.rstrip('.mtl')
            var_name = weapon_skin_name.split('_')[1]
            tree = ET.parse(path)
            root = tree.getroot()
            ...
            if item_exists == True:
                ...
                ET.SubElement(root.find('materials'), 'material',
                            name=f"{var_name}",
                            file=f"objects/attachments/{weapon_folder}/{x}",
                            tpfile=f"objects/attachments/{weapon_folder}/{weapon_skin_name}_tp.mtl")

            with open(path, 'w') as mod_xml:
                mod_xml.write(pretty_print(root))
        mtl_list.clear()

# ----------------------------------------------------------------------------
    if  item == 'bp07':
        # "bp07_01" is the source name for bp07 attachment, chaos within
        plan_b = '\\' + f'{item}' + '\\' + f'{item}_01.xml'
        crazy_source = client_att + plan_b
        crazy_destination = rep_att + plan_b
        shutil.copyfile(crazy_source, crazy_destination)
        _next_step(crazy_destination, mtl, item)
    
    else:   
        try:
            shutil.copyfile(client_xml, rep_xml)
            _next_step(rep_xml, mtl, item)

        except(FileNotFoundError):
            os.makedirs(path_skin + f'\\Objects\Attachments\\{item}')
            shutil.copyfile(client_xml, rep_xml)
            _next_step(rep_xml, mtl, item)

#------------------------------------------------------------------------------
#                   att_No_Skin_xml, add_material_att, att_Skin_xml
#------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------
#                           Game\Items\Accessories + "bp07"fix
#------------------------------------------------------------------------------
        ...
        for m in mtl:
            ...
            for file in glob.glob(f"{raw_gun}0{num}*.xml"):
                if len(file) > 9:
                    ...
                    break
            #---------
            path = acc_att + "\\" + raw_gun + f'0{num}.xml'    # 'c:\\wfc\\Game\\Items\\Accessories\\bp02.xml'
            a_path = acc_att + "\\" + raw_gun + f'0{num}_' + xml    # 'c:\\wfc\\Game\\Items\\Accessories\\bp02_camo08.xml'
            b_path = rep_acc + "\\" + f'{skin_item}.xml'    # 'd:\\_WFC_repo\\...\\Game\\Items\\Accessories\\bp07_rock00001.xml'
